...
void PrintCities(TSP ProblemSet, Population Pop, int generation, int rank) {

   string rank_ = to_string(rank);
   string num_str = to_string(generation);
   ofstream Output("output/NoMigrations/Cities"+rank_+"/Output.gen"+num_str+".txt");

   int dim = ProblemSet.GetCities().size();

   for(int i=0; i<dim; i++) {
      int index_order = Pop.Get_i_Chrom(0).Get_i_gen(i);
      Output << std::setprecision(8) << ProblemSet.GetCities()[index_order].GetX() << setw(20) << ProblemSet.GetCities()[index_order].GetY() << endl;
   }
...
void PrintBestFitness(TSP ProblemSet, Population Pop, int generation, int rank) {

   string rank_ = to_string(rank);
   ofstream Output("output/NoMigrations/Output.BestFitness"+rank_+".txt", ios::app);
...
void PrintBestAvFitness(TSP ProblemSet, Population Pop, int generation, int rank) {

   string rank_ = to_string(rank);
   ofstream Output("output/NoMigrations/Output.BestAvFitness"+rank_+".txt", ios::app);
