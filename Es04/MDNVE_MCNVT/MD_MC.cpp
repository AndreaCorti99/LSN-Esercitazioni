...
{ 
  ...
  nblk = 20000; //modifico i parametri rispetto a quelli che serviranno poi per la simulazione
  nstep = 1;
  eq = "/eq" ;
  int nconf = 1; 
  ...
  //SIMULAZIONE
  nblk = 30; //modifico i parametri rispetto a quelli che serviranno poi per la simulazione
  nstep = 2000;
  eq = "" ;
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=20;
    ...
    Epot.open("output/"+phase+eq+"/output_epot.dat",ios::app);
    Ekin.open("output/"+phase+eq+"/output_ekin.dat",ios::app);
    Temp.open("output/"+phase+eq+"/output_temp.dat",ios::app);
    Etot.open("output/"+phase+eq+"/output_etot.dat",ios::app);
    Pres.open("output/"+phase+eq+"/output_pres.dat",ios::app);
...
void ConfFinal(void)
{
  ...
  WriteConf.open("output/"+phase+eq+"/config.out");
  WriteVelocity.open("output/"+phase+eq+"/velocity.out");
  for (int i=0; i<npart; ++i)
  {
